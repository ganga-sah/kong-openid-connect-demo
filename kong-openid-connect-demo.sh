...
echo Add a Service
http put ${KONG_ADMIN_API_URI}/services/${ROUTE}-service url=http://httpbin.org/anything | jq
...
http get ${KONG_SERVICE_URI}/${ROUTE} | jq
...
config.ssl_verify=false | jq

echo Provide a client_id and secret to retrieve a token
http get ${KONG_SERVICE_URI}/${ROUTE} -a ${CLIENT_ID}:${CLIENT_SECRET} | jq
...
http get ${KONG_SERVICE_URI}/${ROUTE} -a user:password | jq
...
http get ${KONG_SERVICE_URI}/${ROUTE} -a ${USERNAME}:${PASSWORD} | jq
...
http get ${KONG_SERVICE_URI}/${ROUTE} Authorization:"$BEARER_TOKEN" | jq
...
http get ${KONG_SERVICE_URI}/${ROUTE} -a partner:test | jq
...
http get ${KONG_SERVICE_URI}/${ROUTE} username=${USERNAME} password=${PASSWORD}
http get ${KONG_SERVICE_URI}/${ROUTE} username=${USERNAME} password=${PASSWORD}
http get ${KONG_SERVICE_URI}/${ROUTE} username=${USERNAME} password=${PASSWORD}
http get ${KONG_SERVICE_URI}/${ROUTE} username=${USERNAME} password=${PASSWORD}
http get ${KONG_SERVICE_URI}/${ROUTE} username=${USERNAME} password=${PASSWORD}
